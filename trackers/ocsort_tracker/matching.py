...
def fuse_motion(kf, cost_matrix, tracks, detections, only_position=True, lambda_=0.98):
    ...
    for row, track in enumerate(tracks):
        # print(f'covar: {np.diag(track.covariance)}')
        gating_distance = kf.gating_distance(
            track.mean, track.covariance, measurements, only_position, metric='maha')
        # print(f'gating_distance: {gating_distance}')
        cost_matrix[row, gating_distance > gating_threshold] = np.inf
...
def get_dists(
    tracks, detections, _fuse_score:bool,
    is_reid:bool, feature_thresh=0.8, proximity_thresh=0.5
):
    dists = iou_distance(tracks, detections)
    dists_mask = dists > (1 - proximity_thresh)
    ...
    if is_reid:
        ...
        emb_dists[emb_dists > (1 - feature_thresh)] = 1.0
